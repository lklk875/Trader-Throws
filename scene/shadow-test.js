...
const { vec3, vec4, color, Mat4, Light, Shape, Material, Shader, Texture, Scene, Webgl_Manager } = tiny;
...
export class Test_Game_Scene extends Scene {
  constructor(context, control_box) // The scene begins by requesting the camera, shapes, and materials it will need.
  {
    ...
    shadow: new Material( new defs.Shadow_Shader(), { ambient: 0.5, diffusivity: 0, specularity: 0, color: color(204/255,154/255,154/255,1) }),
...
//     this.boxes = this.set_boxes(); 
  ...
  make_control_panel() // Draw the scene's buttons, setup their actions and keyboard shortcuts, and monitor live measurements.
  {
    ...
    this.key_triggered_button( "normal view", [ "5" ], ()=> { this.program_state.set_camera( Mat4.translation( -1,-8,-25 )); 
                                                              window.ready = true;
  ...
  in_bin(bin_num, ball_y, ball_z) {
    ...
    {   
          if (this.challenge)
          {
            ...
            if(!this.game_over && this.time < 60 && this.score >= 1)
            {
              this.game_end = true;
              this.reset_se();
              this.reset_bg();
          ...
          else
          {
            ...
            if(!this.game_over && this.time < 60 && this.life > 0 && this.score >= 1)
            {
              this.game_end = true;
              this.reset_se();
              this.reset_bg();
    ...
    if(this.life == 0 || this.game_over || this.time > 10)
    {
      /*if(this.time > 10)
      {
        this.curr_sound_effect = this.sounds.timeover; 
        this.curr_sound_effect.load();
        this.curr_sound_effect.play();
      }
      */
      ...
      this.reset_se();
      this.reset_bg();
      this.curr_sound_effect = this.sounds.gameover_music;
      this.curr_sound_effect.load();
      this.curr_sound_effect.play();
    }
  }
  ...
  { 
    ...
    const seconds = (this.t - this.start_seq_t) / 1000 + 10; 
...
//     intro scene 2: Around store 
    ...
    //intro scene 4: LEFT side of store
    ...
    if (window.pan && seconds >= 20 && seconds < 30)
       program_state.set_camera( camera_matrix );
...
// ****************** END CAMERA PAN SECTION ******************/
    ...
    tr_bin2_shadow = tr_bin2_shadow.times(Mat4.scale(3, 0.01, 3.5));
    tr_bin2_shadow = tr_bin2_shadow.times(Mat4.translation(0, 1.05, -10.4));
    tr_bin2_shadow = tr_bin2_shadow.times(Mat4.rotation(-Math.PI/2, 0,1,0));
    tr_bin2_shadow = tr_bin2_shadow.times(Mat4.translation(0, 0, -6));
    tr_bin2_shadow = tr_bin2_shadow.times(Mat4.shear(1));

    let tr_bin0_shadow = Mat4.identity();
    tr_bin0_shadow = tr_bin0_shadow.times(Mat4.scale(3, 0.01, 3.5));
    tr_bin0_shadow = tr_bin0_shadow.times(Mat4.translation(0, 1.05, -10.4));
    tr_bin0_shadow = tr_bin0_shadow.times(Mat4.rotation(-Math.PI/2, 0,1,0));
    tr_bin0_shadow = tr_bin0_shadow.times(Mat4.translation(0, 0, 6));
    ...
    tr_shelf1 = tr_shelf1.times(Mat4.translation(-30,6,0));
...
//     this.shapes.bin.draw(context, program_state, this.tr_bin2, this.materials.bin2);
    ...
    //shadows
    this.shapes.bin.draw(context, program_state, tr_bin0_shadow, this.materials.shadow);
    this.shapes.bin.draw(context, program_state, tr_bin1_shadow, this.materials.shadow);
    this.shapes.bin.draw(context, program_state, tr_bin2_shadow, this.materials.shadow);
    ...
    let messageModelTransform = this.initial_camera_location.times(Mat4.translation(-14,7.5,-15.5)) // [-11,5.5,-15] for scoreboard
     ...
     // if(this.game_over)
      if(this.game_over || this.time >= 60 && this.score < 30 && !this.game_end)
      {
          this.life = 0;
...
//           program_state.set_camera( Mat4.translation(0, 0, 50).times(Mat4.rotation(Math.PI, 0, 1, 0)) );
      ...
      //display Game Ending
      else if(!this.game_over && this.time < 60 && this.life > 0 && this.score >= 1) //this.score should be over 30, I set this.score >= 1 for testing
      {
          this.shapes.plane.draw(context, program_state, tr_intro_pink, this.materials.pink_wall); 
          this.shapes.plane.draw(context, program_state, tr_intro_white, this.materials.white_wall);  
...
//           program_state.set_camera( Mat4.translation(0, 0, 50).times(Mat4.rotation(Math.PI, 0, 1, 0)) );
      ...
      let vegBoxL = Mat4.translation(-40, 4, -30).times(Mat4.rotation(Math.PI/2, 0, 1, 0)).times(Mat4.scale(10,4,6));
      let vegBoxR = Mat4.translation(35, 4, -20).times(Mat4.rotation(Math.PI/2, 0, 1, 0)).times(Mat4.scale(12,4,6));
//       let cashReg = Mat4.translation(15, 3, -65).times(Mat4.scale(15,4,5));

      let shelf1 = Mat4.translation(-75, 10, 0).times(Mat4.scale(12,10,10));
      let shelf2 = Mat4.translation(-75, 10, -35).times(Mat4.scale(12,10,10));
      let shelf3 = Mat4.translation(-75, 10, -70).times(Mat4.scale(12,10,10));
...
//       this.shapes.cube.draw(context, program_state, cashReg, this.materials.white_wall);
      ...
      this.shapes.crate.draw(context, program_state, shelf1, this.materials.white_wall);
      this.shapes.cube.draw(context, program_state, shelf2, this.materials.white_wall);
      this.shapes.cube.draw(context, program_state, shelf3, this.materials.white_wall);
      ...
      for (k; k < this.boxes.length; k++) 
            this.shapes.cube.draw(context,  program_state, this.boxes[k], this.materials.crate);
...
//         let pickMe = Math.floor(Math.random() * 3); //0, 1, 2
        ...
        let box1, box2, box3, box4, box5;
           if (pickMe == 2) {
               ...
               box2 = box1.times(Mat4.scale(0.5, 0.5, 0.5).times(Mat4.translation(3, 0, 0)).times(Mat4.rotation(Math.PI/6, 1, 0, 0 )));

               box3 = Mat4.translation(-30, 4, -50);
               box3 = box3.times(Mat4.scale(3,4,3));

               box4 = Mat4.translation(-15, 3, -55).times(Mat4.rotation(Math.PI/6, 0, 1, 0)).times(Mat4.scale(2, 3, 2));

               box5 = Mat4.translation(-7, 3, -50).times(Mat4.rotation(Math.PI/2, 0, 0, 1)).times(Mat4.scale(1, 2, 1));
//                box5 = Mat4.translation(0, yStretch, 0).times(Mat4.scale(yStretch, yStretch, yStretch)); 
           }
           else if (pickMe) { //1 
